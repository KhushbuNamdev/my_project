...
export const createUser = async (req, res) => {
  try {
    ...
    if ((role === 'wholesaler' || role === 'retailer') && req.user.role !== 'superadmin') {
      return res.status(403).json({
        success: false,
        message: 'Only superadmin can create business accounts',
      });
    }
    ...
    // Add business account fields for wholesaler/retailer
    if (['wholesaler', 'retailer'].includes(role)) {
      userData.businessName = businessName;
      userData.gstNumber = gstNumber;
      if (role === 'retailer' && businessType) {
    ...
    if (user) {
      ...
      const responseData = {
        ...
        ...((user.role === 'wholesaler' || user.role === 'retailer') && {
          ...
          ...(user.role === 'retailer' && { businessType: user.businessType }),
